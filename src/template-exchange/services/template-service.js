...
async function listTemplates(options = {}) {
  try {
    const { type, status, limit = 10, startAfterId = null } = options;
    
    // Get starting document for pagination if provided
    let startAfterDoc = null;
    if (startAfterId) {
      startAfterDoc = await templateModel.getById(startAfterId);
      if (!startAfterDoc.exists) {
        throw new Error(`Start document with ID ${startAfterId} not found`);
      }
    }
    
    // If filtering by type, use the specific method
    if (type && !status) {
      const templates = await templateModel.getByType(type, limit, startAfterDoc);
    ...
    // For other combinations, we'll need to implement custom queries
    // that leverage the sharding strategy
    
    // Start with the first shard
    const shards = templateModel.shards || ['a', 'b', 'c'];
    const queries = [];
    
    for (const shard of shards) {
      let query = db.collection('templates').where('shard', '==', shard);
      ...
      // Apply limit to each shard query
      query = query.limit(limit);
      
      // Use startAfter for pagination if provided
      if (startAfterDoc) {

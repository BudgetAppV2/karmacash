...
// Process a single B5 document through all 6 steps
async function processB5Document(fileName) {
  console.log(`\n=== Processing ${fileName} ===\n`);
  
  const docId = fileName.replace('.md', '').replace('-', '_');
  const sectionId = docId.replace(/_/g, '.');
...
async function uploadRawFile(fileName) {
  ...
  const storagePath = `bible/sections/b5/raw/${fileName}`;
...
async function createFirestoreMetadata(docId, sectionId, uploadResult) {
  const metadata = {
    title: docId.replace(/_/g, ' ').replace('B5.', ''),
    document_id: docId,
...
async function generateContentAnalysis(fileName, docId) {
  ...
  const analysis = {
    ...
    dominant_content_types: ['database_design', 'data_schemas', 'security_rules'],
    ...
    potential_for_ai_optimization: 'High - structured database content ideal for schema generation',
...
async function createSemanticChunks(fileName, docId) {
  ...
  for (const chunk of chunks) {
    const chunkPath = `bible/sections/b5/chunks/${docId}/${chunk.chunk_id}.json`;
...
async function createOptimizedJson(fileName, docId, analysis, chunkIds) {
  ...
  const optimized = {
    ...
    chunk_mapping: chunkIds.map(id => ({
      chunk_id: id,
      storage_path: `bible/sections/b5/chunks/${docId}/${id}.json`
    })),
    metadata: {
  ...
  const optimizedPath = `bible/sections/b5/optimized/${docId}.optimized.json`;
...
async function updateMetadataCompletion(docId, chunkCount, crossRefCount) {
  await db.collection('bible_storage_metadata').doc(docId).update({
    ...
    optimized_json_path: `bible/sections/b5/optimized/${docId}.optimized.json`,
...
async function main() {
  const documents = [
    'B5.1_Schema_Overview.md',
    'B5.2_Collections_Schemas.md',
    'B5.3-security-rules-strategy.md'
  ];
  ...
  for (const doc of documents) {
    try {
      const result = await processB5Document(doc);
      results.push(result);
    } catch (error) {

...
// Process a single B6 document through all 6 steps
async function processB6Document(fileName) {
  console.log(`\n=== Processing ${fileName} ===\n`);
...
async function uploadRawFile(fileName) {
  ...
  const storagePath = `bible/sections/b6/raw/${fileName}`;
...
async function createFirestoreMetadata(docId, sectionId, uploadResult) {
  const metadata = {
    title: docId.replace(/_/g, ' ').replace('B6.', ''),
    document_id: docId,
...
async function generateContentAnalysis(fileName, docId) {
  ...
  const analysis = {
    ...
    dominant_content_types: ['business_logic', 'algorithms', 'calculations', 'budget_processing'],
    ...
    potential_for_ai_optimization: 'High - algorithmic content ideal for code generation and optimization',
...
async function createSemanticChunks(fileName, docId) {
  ...
  for (const chunk of chunks) {
    const chunkPath = `bible/sections/b6/chunks/${docId}/${chunk.chunk_id}.json`;
...
async function createOptimizedJson(fileName, docId, analysis, chunkIds) {
  ...
  const optimized = {
    ...
    chunk_mapping: chunkIds.map(id => ({
      chunk_id: id,
      storage_path: `bible/sections/b6/chunks/${docId}/${id}.json`
    })),
    metadata: {
  ...
  const optimizedPath = `bible/sections/b6/optimized/${docId}.optimized.json`;
...
async function updateMetadataCompletion(docId, chunkCount, crossRefCount) {
  await db.collection('bible_storage_metadata').doc(docId).update({
    ...
    optimized_json_path: `bible/sections/b6/optimized/${docId}.optimized.json`,
...
async function main() {
  const documents = [
    'B6.1_Budget_Calculations.md',
    'B6.2_Recurring_Algorithm.md'
  ];
  ...
  for (const doc of documents) {
    try {
      const result = await processB6Document(doc);
      results.push(result);
    } catch (error) {

...
// Process a single B7 document through all 6 steps
async function processB7Document(fileName) {
  console.log(`\n=== Processing ${fileName} ===\n`);
  
  const docId = fileName.replace('.md', '').replace(/-/g, '_');
...
async function createFirestoreMetadata(docId, sectionId, uploadResult) {
  const metadata = {
    title: docId.replace(/_/g, ' ').replace('B7.', ''),
    document_id: docId,
...
async function generateContentAnalysis(fileName, docId) {
  ...
  const analysis = {
    ...
    dominant_content_types: ['workflow_processes', 'ai_integration', 'testing_strategies', 'development_practices'],
    ...
    potential_for_ai_optimization: 'High - workflow and process content ideal for automation and AI assistance',
...
async function main() {
  const documents = [
    ...
    'B7.1-AI-Workflow-SHIP.md',
    'B7.2_SHIP_Template_TaskMaster_Integrated.md',
    'B7.2_Testing_Strategy.md',
    'B7.3_Decision_Log.md',
    'B7.3_Manus_AI_Feature_Breakdown_Prompt_Template.md',
    'B7.4_AI_Mockup_Prompt_Template.md',
  ...
  for (const doc of documents) {
    try {
      const result = await processB7Document(doc);
      results.push(result);
    } catch (error) {

...
// Process a single B8 document through all 6 steps
async function processB8Document(fileName) {
  console.log(`\n=== Processing ${fileName} ===\n`);
...
async function uploadRawFile(fileName) {
  ...
  const storagePath = `bible/sections/b8/raw/${fileName}`;
...
async function createFirestoreMetadata(docId, sectionId, uploadResult) {
  const metadata = {
    title: docId.replace(/_/g, ' ').replace('B8.', ''),
    document_id: docId,
...
async function generateContentAnalysis(fileName, docId) {
  ...
  const analysis = {
    ...
    dominant_content_types: ['setup_guide', 'installation_instructions', 'project_configuration'],
    ...
    potential_for_ai_optimization: 'High - setup and configuration content ideal for automation and scripting',
...
async function createSemanticChunks(fileName, docId) {
  ...
  for (const chunk of chunks) {
    const chunkPath = `bible/sections/b8/chunks/${docId}/${chunk.chunk_id}.json`;
...
async function createOptimizedJson(fileName, docId, analysis, chunkIds) {
  ...
  const optimized = {
    ...
    chunk_mapping: chunkIds.map(id => ({
      chunk_id: id,
      storage_path: `bible/sections/b8/chunks/${docId}/${id}.json`
    })),
    metadata: {
  ...
  const optimizedPath = `bible/sections/b8/optimized/${docId}.optimized.json`;
...
async function updateMetadataCompletion(docId, chunkCount, crossRefCount) {
  await db.collection('bible_storage_metadata').doc(docId).update({
    ...
    optimized_json_path: `bible/sections/b8/optimized/${docId}.optimized.json`,
...
async function main() {
  ...
  for (const doc of documents) {
    try {
      const result = await processB8Document(doc);
      results.push(result);
    } catch (error) {

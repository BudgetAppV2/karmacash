...
// Process a single B4 document through all 6 steps
async function processB4Document(fileName) {
  console.log(`\n=== Processing ${fileName} ===\n`);
...
async function uploadRawFile(fileName) {
  ...
  const storagePath = `bible/sections/b4/raw/${fileName}`;
...
async function createFirestoreMetadata(docId, sectionId, uploadResult) {
  const metadata = {
    title: docId.replace(/_/g, ' ').replace('B4.', ''),
    document_id: docId,
...
async function generateContentAnalysis(fileName, docId) {
  ...
  const analysis = {
    ...
    dominant_content_types: ['ui_ux_design', 'component_specifications', 'style_guidelines'],
    ...
    potential_for_ai_optimization: 'High - structured UI/UX content ideal for component generation',
...
async function createSemanticChunks(fileName, docId) {
  ...
  for (const chunk of chunks) {
    const chunkPath = `bible/sections/b4/chunks/${docId}/${chunk.chunk_id}.json`;
...
async function createOptimizedJson(fileName, docId, analysis, chunkIds) {
  ...
  const optimized = {
    ...
    chunk_mapping: chunkIds.map(id => ({
      chunk_id: id,
      storage_path: `bible/sections/b4/chunks/${docId}/${id}.json`
    })),
    metadata: {
  ...
  const optimizedPath = `bible/sections/b4/optimized/${docId}.optimized.json`;
...
async function updateMetadataCompletion(docId, chunkCount, crossRefCount) {
  await db.collection('bible_storage_metadata').doc(docId).update({
    ...
    optimized_json_path: `bible/sections/b4/optimized/${docId}.optimized.json`,
...
async function main() {
  const documents = [
    'B4.1_Tech_Stack_Details.md',
    'B4.2_Frontend_Structure.md',
    'B4.3_Backend_Functions.md',
    'B4.4_Logging_System_Impl.md'
  ];
  ...
  for (const doc of documents) {
    try {
      const result = await processB4Document(doc);
      results.push(result);
    } catch (error) {

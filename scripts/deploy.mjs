 ...
 * 
 * Usage:
 *   node scripts/deploy.mjs [--rules] [--indexes] [--functions] [--all]
 * 
 * Options:
...
function checkFirebaseCLI() {
  try {
    execCommand('firebase --version', { stdio: 'pipe' });
  } catch (error) {
    console.error(chalk.red('Firebase CLI not found. Please install it with:'));
...
async function deployRules() {
  ...
  try {
    ...
    execCommand(`firebase deploy --only firestore:rules --project=${projectId}`, { stdio: 'inherit' });
...
async function deployIndexes() {
  ...
  try {
    const projectId = getProjectId();
    execCommand(`firebase deploy --only firestore:indexes --project=${projectId}`, { stdio: 'inherit' });
...
async function deployFunctions() {
  ...
  try {
    ...
    if (!fs.existsSync(nodeModulesPath)) {
      ...
      execCommand('npm install', { cwd: config.functionsDir, stdio: 'inherit' });
    }
    
    const projectId = getProjectId();
    execCommand(`firebase deploy --only functions --project=${projectId}`, { stdio: 'inherit' });
